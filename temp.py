...
class Pokemon:
    ...
    def level_up(self, nb_lv=1):
        ...
        self.pv = round((round((2 * int(
            self.line[3]) * self.level) / 100 + self.level + 10) + self.bonus_pvmax) * self.multiplicateur_pvmax)
        self.health = self.pv - diff
        self.base_attack = round(
            (round((2 * int(self.line[4]) * self.level) / 100 + 5) + self.bonus_attack) * self.multiplicateur_attack)
        self.base_defense = round(
            (round((2 * int(self.line[5]) * self.level) / 100 + 5) + self.bonus_defense) * self.multiplicateur_defense)
        self.base_speed = round(
            (round((2 * int(self.line[6]) * self.level) / 100 + 5) + self.bonus_speed) * self.multiplicateur_speed)
